...
Product_order::Product_order(std::string name, double price, int quantity) : Product{name, price}, _quantity{quantity} {}
Product_order::Product_order(Product product, int quantity) : Product{product}, _quantity{quantity} {}
...
{
    return ((name()==po.name()) && (price()==po.price()) && (quantity()==po.quantity()));
}
bool Product_order::operator!=(const Product_order& po) 
{
    return !(*this == po);
